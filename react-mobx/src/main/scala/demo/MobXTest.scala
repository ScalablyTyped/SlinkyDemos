...
object MobXTest {
  ...
  val component: FunctionalComponent[Props] = ObservingFC[Props] { case Props(store) =>
    val increaseNum = () => {
    ...
    div(
      Mui.Avatar(icon = Mui.ActionAlarm(hoverColor = "blue")(onClick := { (e: SyntheticMouseEvent[`*`.tag.RefType]) =>
        console.warn(s"icon clicked ${e.nativeEvent}")
